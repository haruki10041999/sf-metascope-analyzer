...
export type NewExpressionType = {
    ...
    | {
          ...
          initializeExpression?: (
              | Omit<ExpressionType, 'type'>
              | { key: Omit<ExpressionType, 'type'>; value: Omit<ExpressionType, 'type'> }
          )[];
      }
);
...
export const makeNewExpressionType = (ctx: NewExpressionContext): NewExpressionType => {
    ...
    if (ctx.creator().arrayCreatorRest()) {
        ...
        if (ctx.creator().arrayCreatorRest().expression()) {
            const { type, ...size } = new ExpressionVisitor().visit(
                ctx.creator().arrayCreatorRest().expression(),
            );
            instanceField.size = size;
        }
        ...
        if (ctx.creator().arrayCreatorRest().arrayInitializer()) {
            newExpressionType.initializeExpression = ctx
                ...
                .expression_list()
                .map((expressionCtx) => {
                    const { type, ...init } = new ExpressionVisitor().visit(expressionCtx);
                    return init;
                });
        }
