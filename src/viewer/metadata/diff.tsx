...
import { MetadataCommonType, MetadataObjectDiff } from '../../analyzer/types';
...
export type FieldDefinition = {
    ...
    name: string;
    type: string;
    relationType?: 'lookup' | 'masterDetail'; // 参照か主従か
    targetObjectId?: string; // 接続先のオブジェクトID
};
...
export type FieldNode = {
    name: string;
    type: string;
    relationType: 'lookup' | 'masterDetail';
};
...
// 2. 項目用カスタムノード（小さな四角）
const FieldNodeComponent: React.FC<NodeProps<FieldNodeData>> = ({ data }) => {
    const isMaster = data.relationType === 'masterDetail';
    ...
    return (
        ...
        >
            ...
            <div style={{ fontWeight: 'bold', color: '#1e293b' }}>{data.name}</div>
